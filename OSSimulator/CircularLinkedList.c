...
void printData(circularlistnode *doneStart, circularlistnode *pointer) {
    ...
    printf("pid: %d entrytime: %d runtime: %d timeEnteredReadyQ: %d timeEnteredCPU: %d timeComplete: %d totalTimeInSystem: %d\n",(pointer->current)->pID, (pointer->current)->entryTime, (pointer->current)->runTime, (pointer->current)->timeEnteredReadyQ, (pointer->current)->timeEnteredCPU, (pointer->current)->timeDone, pointer->current->timeDone - pointer->current->entryTime);
